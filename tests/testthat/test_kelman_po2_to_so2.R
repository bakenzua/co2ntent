...
test_that("kelman_po2_to_so2 returns correct number", {
  expect_equal(
    kelman_po2_to_so2(10, 37, 7.4, 5.3329, pressure_units = "kPa"),
    0.950795,
    tolerance = 0.000001
  )
})
...
test_that("kelman_po2_to_so2 returns correct number for mmHg", {
  expect_equal(
    kelman_po2_to_so2(75.006157584565642, 37, 7.4, 40, pressure_units = "mmHg"),
    0.950795,
    tolerance = 0.000001
  )
})
...
test_that("kelman_po2_to_so2 returns is vectorised", {
  ...
  expect_equal(
    kelman_po2_to_so2(po2s, 37, 7.4, 5.3329),
    expected,
    tolerance = 0.000001
  )
})
...
test_that("kelman_po2_to_so2 returns is vectorised, multiple params", {
  ...
  expect_equal(
    kelman_po2_to_so2(po2s, ts, phs, co2s),
    expected,
    tolerance = 0.000001
  )
})
