...
vignette: >
  %\VignetteIndexEntry{Calculating SO2 from pO2}
...
The three functions presented here are based on work of @kelman_1966. This model is inacccurate as $p_{O_2}$ approaches 0, but is otherwise an excellent fit to physiologically occuring values.
...
`co2ntent::kelman_std_po2_to_so2` calculates $S_{O_2}$ from $p_{O_2}$ in blood via the seven coefficient model described above. This function makes no correction for acid/base disturbance, assuming pH=7.4, temperature=37c and pCO2=40mmHg
...
* temperature	- temperature in celcius. Default 37c
* ph - pH (hydrogen ion concentration). Default 7.40
* pressure_units - Input parameters are kPa, otherwise use mmHg. Default kPa

### po2_to_so2

`po2_to_so2(po2, temperature = 37, ph = 7.4, pco2 = 5.332895, pressure_units = c("kPa", "mmHg"))`

At this point `po2_to_so2' is a wrapper function for `kelman_po2_to_so2` as only Kelman's method is implemented.
